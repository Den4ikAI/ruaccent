...
class TagsPredictor:
    ...
    def predict_tags(self, text):
        inputs = self.tokenizer(text, return_offsets_mapping=True, return_special_tokens_mask=True, is_split_into_words=True, return_tensors="np")
        ...
        text_c = self.tokenizer.decode(input_ids, skip_special_tokens=True).replace(" =","=")
        ...
        pre_entities = self.collect_pre_entities(text_c, input_ids, scores[0], offset_mapping, special_tokens_mask)
